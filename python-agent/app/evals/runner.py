...
def score_case(output: str, warnings: List[str], case: Dict[str, Any], run_metadata: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    grounded_ok = True if not grounded_required else bool((run_metadata or {}).get('grounded', False))
    passed = len(required_hits) == len(required) and len(forbidden_hits) == 0 and grounded_ok
    ...
    return {'passed': passed, 'required_keywords': required, 'required_hits': required_hits, 'forbidden_keywords': forbidden, 'forbidden_hits': forbidden_hits, 'warning_count': len(warnings), 'failure_category': failure_category}
...
def run_all_evals() -> Dict[str, Any]:
    ...
    eval_run = {'summary': {'total_cases': total, 'passed_cases': passed, 'failed_cases': total - passed, 'pass_rate': round((passed / total) if total else 0.0, 4), 'failure_counts': failure_counts}, 'results': results, 'dataset': 'golden_eval_set.json'}
